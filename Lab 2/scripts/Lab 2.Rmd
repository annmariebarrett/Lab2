...
plot = tv_states_long %>%
  ...
  geom_vline(aes(xintercept = as.Date(tv_states_long$date[127])), linetype="dotted", color = "grey90", size=.3)+ #third line
   annotate( x=as.Date(tv_states_long$date[127]),y=3,label= 'atop(italic("Las Vegas"), italic("shooting"))', geom="text", color = "grey", parse = TRUE, size = 3, family = 'mono') #second anotate #HOW TO CHANGE LINE SPACE BETWEEN HARVEY LANDFALL
